...
Options:
  ...
  --top=<N>        Show only top N contributors [default: all].
  ...
  --summary         Show one-line summary instead of full table.
...
def run(args):
    ...
    # luogit enhancement: --summary one-liner
    if getattr(args, 'summary', False):
        n = len(auth_stats); tot_loc = stats_tot.get('loc',0)
        top = sorted(auth_stats.items(), key=lambda x: x[1].get('loc',0), reverse=True)
        top_auth = top[0] if top else ('none',{})
        print(f"luogit: {n} author(s), {stats_tot.get('files',0)} file(s), {tot_loc} LOC, {stats_tot.get('commits',0)} commit(s)")
        if top and n > 1:
            p2 = top[1] if len(top) > 1 else top[0]
            diff = top[0][1].get('loc',0) - p2[1].get('loc',0)
            print(f"  top: {top_auth[0][:30]} {top_auth[1].get('loc',0)} LOC, +{diff} over #{2}")
        return
    # luogit enhancement: --top N
